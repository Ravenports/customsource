...
def main():
    ...
    if portname:
        ...
        ravensrc = script_dir.parent.parent  # /ravensource/
        port_dir = os.path.join(ravensrc, bucket, portname)
    ...
    if portname and version:
        ...
        extract_subdir = os.path.join(extract_dir, f"{portname}-{version}")
    ...
    if portname and version:
        ...
        print("Collecting module dependency information...")
        # Temporarily run go mod vendor in temp dir to get fresh modules.txt
        # (in case original didn't have all dependencies)
        temp_vendor_dir = os.path.join(extract_subdir, "vendor_temp")
        Path(temp_vendor_dir).mkdir(parents=True, exist_ok=True)
        ...
        if result.returncode == 0:
            print(f"Found {len(result.stdout.strip().split(chr(10)))} modules")
            ...
            # Set distfiles for later use
            all_module_data = list(result.stdout.strip().split("\n"))
        else:
            print(f"Warning: Could not list dependencies: {result.stderr}")
            sys.exit(1)
